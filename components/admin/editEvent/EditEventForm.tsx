...
import { DragAndDropUploader } from "../createEvent/ImageDragAndDrop";
import { useUploadImage } from "@/hooks/useUploadImage";
import { Event } from "@prisma/client";
import { editEventAction } from "@/lib/actions/editEventAction";

type CreateEventFormInput = z.infer<typeof CreateEventSchema>;

export default function EditEventForm({ event }: { event: Event }) {
  const { categories } = useCategories();
  const [hasMaxCapacity, setHasMaxCapacity] = useState(!!event.maxCapacity);
  ...
  const { handleImageUpload, isUploading, setIsUpLoading } =
    useUploadImage(selectedImage);
  ...
  const {
    ...
    defaultValues: {
      ...
      maxCapacity: event.maxCapacity ? event.maxCapacity : 1,
      image: event.image,
      date: event.date,
    },
  });
  ...
  const onSubmit = (data: CreateEventFormInput) => {
    ...
    startTransition(async () => {
      ...
      try {
        const updatedEvent = await editEventAction(data, event.id);

        if (updatedEvent) {
          alert("Event updated");
          router.push(`/admin/events/${updatedEvent.id}`);
        }
      } catch (error) {
        console.log(error);
